<template>
  <div class="page-container">
    ...
    <div v-else-if="photos.length === 0" class="empty-container">
      <el-empty description="相册暂无照片，快去上传吧">
    ...
    <el-dialog v-model="showMoveDialog" title="移动到相册" width="400px">
      <el-radio-group v-model="targetAlbumId">
        <el-radio v-for="album in otherAlbums" :key="album.id" :label="album.id">
          {{ album.name }}
        </el-radio>
      </el-radio-group>
      <template #footer>
    ...
    <!-- 复制到相册对话框 -->
    <el-dialog v-model="showCopyDialog" title="复制到相册" width="400px">
      <el-radio-group v-model="targetAlbumId">
        <el-radio v-for="album in allAlbums" :key="album.id" :label="album.id">
          {{ album.name }}
        </el-radio>
      </el-radio-group>
      <template #footer>
        <el-button @click="showCopyDialog = false">取消</el-button>
        <el-button type="primary" :loading="copying" @click="confirmCopy">
          确定
        </el-button>
      </template>
    </el-dialog>
...
import { ArrowLeft, Plus, View, Edit, Delete, FolderOpened } from '@element-plus/icons-vue'
import { getAlbumPhotos, renamePhoto, movePhoto, copyPhoto, deletePhoto, batchDeletePhotos } from '@/api/photo'
...
const otherAlbums = computed(() => {
  return allAlbums.value.filter(album => album.id !== parseInt(albumId.value))
})

const loadPhotos = async () => {
  loading.value = true
  try {
    const res = await getAlbumPhotos(albumId.value, {
...
const loadAlbums = async () => {
  try {
    ...
    const currentAlbum = allAlbums.value.find(a => a.id === parseInt(albumId.value))
    if (currentAlbum) {
...
const confirmMove = async () => {
  ...
  try {
    for (const photoId of selectedPhotos.value) {
      await movePhoto(photoId, { albumId: targetAlbumId.value })
    }
...
const confirmCopy = async () => {
  ...
  try {
    for (const photoId of selectedPhotos.value) {
      await copyPhoto(photoId, { albumId: targetAlbumId.value })
    }
